...
class Spectra():
    ...
    def _load_data(self):
        ...
        #object_table_path           = '../SNeMPhyVAE/data/spectra/object_ALeRCE_WISeREP_20260115.pkl'
        spectra_alercexwiserep_path = '../SNeMPhyVAE/data/spectra/spectra_ALeRCE_20251226_WISeREP_20260115.pkl'
        ...
        #spectra = pd.merge(left=spectra_alercexwiserep, right=object_table, on='oid', how='inner')
        spectra = spectra_alercexwiserep.copy()

        if self.snia_only:
            snia_labels = [label for label in spectra.type.unique() if 'SN Ia' in label]
            spectra = spectra[spectra.type.isin(snia_labels)]

        if self.snii_only:
            snii_labels = [label for label in spectra.type.unique() if 'SN II' in label]
            spectra = spectra[spectra.type.isin(snii_labels)]
    ...
    def preprocess_spectrum(self, spectra, smooth_spectrum=False):
        ...
        for i, (_, spectrum) in enumerate(tqdm(spectra.iterrows(), total=len(spectra), desc="Processing spectra")):
            ...
            spectra_dict = {
                ...
                #'flux_spline':     np.zeros_like(wave),
                #'flux_continuum':  np.zeros_like(wave),
            ...
            results.append({
                ...
                'spec_units':      spectrum.spec_units,
                'spec_quality':    spectrum.spec_quality,
        ...
        #    ax.plot(row['wave'], row['flux'], label=f'MJD: {row["mjd"]}', alpha=0.5)
